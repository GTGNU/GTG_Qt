...
#ifdef _SDL_TTF_H
	TTF_Font **fonts;
	unsigned int fontsSize, fontsMinSize;
#endif
	bool globalKerning;
	int globalStyle, globalHinting;
	SDL_Surface **surfaces;
	int *surfacesY;
	unsigned int surfacesSize;
	SDL_Color shadedBgColor;
	char renderMode; //0 = blended, 1 = solid, 2 = shaded
...
class GUILabel
{
	...
	GUILabel& operator = (const GUILabel &param);
	void StyleConstruct();
public:
	GUILabel();
	~GUILabel();
...
class GUICheckBox
{
	...
	GUICheckBox& operator = (const GUICheckBox &param);
	void StyleConstruct();
public:
	GUICheckBox();
	~GUICheckBox();
...
class GUIButton
{
	int posX1, posY1, posX2, posY2;
	bool isDisabled, isHovered;
	GUIText renderedText;
	bool isPressed;
	SDL_Color textColor;
	GUIButton& operator = (const GUIButton &param);
	void StyleConstruct();
public:
	GUIButton();
	~GUIButton();
...
class GUIScrollBar
{
	...
	GUIScrollBar& operator = (const GUIScrollBar &param);
	void StyleConstruct();
public:
	GUIScrollBar();
	~GUIScrollBar();
...
class GUITextBox
{
	...
	Uint16 *textUni;
	int textSize;
	GUIText renderedText, *fontGuiText;
	int fontSize, setFontSize;
	Uint32 fontColor, fontBackColor;
	bool fontUseUnicode;
	...
	bool CursorBlink(bool forceShow=false);
	void CurUp();
	void CurDown();
	void CurLeft();
	void CurRight();
	void RemoveChar(bool del);
	void AddChar(char chr, Uint16 uni);
	void RecalculateCursor();
	void Check();
	GUITextBox& operator = (const GUITextBox &param);
	void StyleConstruct();
public:
	GUITextBox();
	~GUITextBox();
...
class GUIListBox
{
	...
	GUIScrollBar scroll;
	char *text;
	int textSize, itemCount, selected, textHeight;
	int curOffX,curOffY;
	bool isMouseDown, showSelectionBox;
