...
void SDLInit()
{
    if(!(SDL_WasInit(SDLHELPER_INIT_FLAGS) & (SDLHELPER_INIT_FLAGS))) {
        if(SDL_Init(SDLHELPER_INIT_FLAGS) == -1) {
...
#ifdef _SDL_NET_H
    if(SDLNet_Init() == -1) {
        fprintf(stderr,"Could not initialize SDL_net: %s",SDLNet_GetError());
        exit(1);
    }
#endif
#ifdef _SDL_TTF_H
    if(TTF_WasInit() == 0)if(TTF_Init() == -1) {
            fprintf(stderr,"Could not initialize SDL_ttf: %s",TTF_GetError());
            exit(1);
        }
#endif
#ifdef _SDL_MIXER_H
    ...
    if((mixFlags&Mix_Init(mixFlags)) != mixFlags) {
        fprintf(stderr,"Could not initialize SDL_mixer: %s",Mix_GetError());
        exit(1);
    }
#endif
...
bool SDLSetVideo(int width, int height, bool resizable, bool fullscreen, bool noframe, bool useOpenGL)
{
    ...
    Uint8 bpp = ((SDL_VideoInfo*)SDL_GetVideoInfo())->vfmt->BitsPerPixel;
    if(bpp>32||(bpp!=32&&bpp!=24&&bpp!=16&&bpp!=8))bpp=32;
...
void SDLEvents::HandleEvent(SDL_Event *event)
{
    ...
    case SDL_ACTIVEEVENT:
        ...
        case SDL_APPMOUSEFOCUS:
            appMouseFocus = (bool)event->active.gain;
            break;
        case SDL_APPINPUTFOCUS:
            appInputFocus = (bool)event->active.gain;
            break;
        case SDL_APPACTIVE:
            appActive = (bool)event->active.gain;
            break;
        }
        break;
    case SDL_KEYDOWN:
    ...
    case SDL_MOUSEBUTTONUP:
        ...
        case SDL_BUTTON_LEFT:
            mouse_l = (event->button.state==SDL_PRESSED)?true:false;
            break;
        case SDL_BUTTON_MIDDLE:
            mouse_m = (event->button.state==SDL_PRESSED)?true:false;
            break;
        case SDL_BUTTON_RIGHT:
            mouse_r = (event->button.state==SDL_PRESSED)?true:false;
            break;
        case SDL_BUTTON_X1:
            mouse_x1 = (event->button.state==SDL_PRESSED)?true:false;
            break;
        case SDL_BUTTON_X2:
            mouse_x2 = (event->button.state==SDL_PRESSED)?true:false;
            break;
            //case SDL_BUTTON_WHEELUP: break;
            //case SDL_BUTTON_WHEELDOWN: break;
        }
        break;
    case SDL_VIDEORESIZE: /* event->resize.w & event->resize.h */
        break;
    ...
    case SDL_QUIT:
        quit = true;
        break;
    case SDL_USEREVENT: /* (int)event->user.code & (void*)event->user.data1 & (void*)event->user.data2 */
        break;
    case SDL_SYSWMEVENT: /* undefined (SDL_SysWMmsg*)event->syswm.msg */
        break;
    }
}
