...
void GTGPlayer::Construct(){
  tileWalkLeft = NULL; tileWalkRight = NULL; tileWalkUp = NULL; tileWalkDown = NULL;
...
void GTGPlayer::Frame(SDLTimer *timer, bool left, bool right, bool up, bool down, GTGMap *map){
  if(isMoving){
    ...
    if(d > 1.){
      isMoving = false;
      prevX = x; prevY = y; drawX = x; drawY = y;
    ...
    if(left){
      ...
      moveTime = timer->now; isMoving = true;
    }else if(right){
      prevX = x; prevY = y;
      if(map->IsSolid(x+1,y))return;
      x++;
