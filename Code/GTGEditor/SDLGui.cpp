...
static struct GUIElement *GUIElementList = NULL;
static size_t GUIElementList_size = 0;
...
void GUIDropDown::RecalculateCoords()
{
    ...
    triX1=posX2-(textBoxHeight/2);
    triY1=posY1+(textBoxHeight*0.625f);
    triX2=triX1-(textBoxHeight/4);
    triY2=triY1-(textBoxHeight/4);
    triX3=triX1+(textBoxHeight/4);
    triY3=triY2;
    arrX1=posX2-textBoxHeight;
    arrY1=posY1;
    arrX2=posX2;
    arrY2=posY1+textBoxHeight;
}
...
void GUIListBox::AddItem(const char *item, int pos)
{
    ...
    textSize=txtSize+itemLen;
    if(txtSize>0)textSize++;
...
void GUITextBox::Construct()
{
    ...
    fontSize=0;
    setFontSize=0;
    fontColor=0x000000FF, fontBackColor=0xFFFFFFFF;
    fontUseUnicode=true;
    textX1=0;
    textY1=0;
    textX2=0;
    ...
    StyleConstruct();
    scrollX.SetHorizontal(true);
    scrollY.SetHorizontal(false);
...
void GUITextBox::CurLeft()
{
    ...
    if(curPos>0)curPos--;
    int pos;
    for(pos=curPos-1; pos>=0; pos--)if(text[pos]=='\n')break;
    CursorBlink(true);
    RecalculateCursor();
}
...
void GUITextBox::RecalculateCursor()
{
    ...
    curDrawX=curX+textX1+curOffX;
    curDrawY=curY+textY1+curOffY;
    curDrawH=(setMaxLines==1)?(textY2-textY1):textHeight;
    if(showScroll) {
...
{
    if(arrowMove<0.f)arrowMove=0.f;
    if(arrowMove>1.f)arrowMove=1.f;
    if(trackMove<0.f)trackMove=0.f;
    if(trackMove>1.f)trackMove=1.f;
...
void GUIScrollBar::RecalculateCoords()
{
    ...
    if(isHorizontal) {
        triAX1=posX1+(arrowW*0.375f);
        triAY1=(posY1+posY2)/2;
        triAX2=triAX1+(arrowW/4);
        triAY2=triAY1-(arrowW/4);
        triAX3=triAX2;
        triAY3=triAY1+(arrowW/4);
        triBX1=posX2-(arrowW*0.375f);
        triBY1=(posY1+posY2)/2;
        triBX2=triBX1-(arrowW/4);
        triBY2=triBY1-(arrowW/4);
        triBX3=triBX2;
        triBY3=triBY1+(arrowW/4);
    } else {
        triAX1=(posX1+posX2)/2;
        triAY1=posY1+(arrowW*0.375f);
        triAX2=triAX1-(arrowW/4);
        triAY2=triAY1+(arrowW/4);
        triAX3=triAX1+(arrowW/4);
        triAY3=triAY2;
        triBX1=(posX1+posX2)/2;
        triBY1=posY2-(arrowW*0.375f);
        triBX2=triBX1-(arrowW/4);
        triBY2=triBY1-(arrowW/4);
        triBX3=triBX1+(arrowW/4);
        triBY3=triBY2;
    }
...
void GUIScrollBar::RecalculateSlider()
{
    if(isHorizontal) {
        ...
        slidX1=trackX1+(scrollPos*(trackX2-trackX1-slidW))+1;
        slidY1=trackY1;
        slidX2=slidX1+slidW-2;
        slidY2=trackY2;
    } else {
        slidW=scrollSize*(trackY2-trackY1);
        slidX1=trackX1;
        slidY1=trackY1+(scrollPos*(trackY2-trackY1-slidW))+1;
        slidX2=trackX2;
        slidY2=slidY1+slidW-2;
    }
}
...
void GUIText::Construct()
{
    ...
    fontsMinSize=0;
#endif
    globalStyle=TTF_STYLE_NORMAL;
    globalHinting=TTF_HINTING_NORMAL;
    globalKerning=true;
    surfaces=NULL;
    surfacesY=NULL;
    surfacesSize=0;
    shadedBgColor.r=0xFF;
    shadedBgColor.g=0xFF;
    shadedBgColor.b=0xFF;
    shadedBgColor.unused=0xFF;
...
int GUIText::Blit(SDL_Surface *dst, int x, int y, int w, int h, bool centerX, bool centerY, int blitOffX, int blitOffY)
{
    ...
    int ret=0, offY=0;
    int lastH=0;
    if(surfaces[surfacesSize-1]!=NULL)lastH=surfaces[surfacesSize-1]->h;
...
{
    SDL_Surface **surfaces_;
    int *surfacesY_;
    unsigned int surfacesSize_;
    SDL_Color shadedBgColor_;
    char renderMode_;
    surfaces_=surfaces;
    surfacesY_=surfacesY;
    surfacesSize_=surfacesSize;
    shadedBgColor_=shadedBgColor;
    renderMode_=renderMode;
    surfaces=guiText->surfaces;
    surfacesY=guiText->surfacesY;
    surfacesSize=guiText->surfacesSize;
    shadedBgColor=guiText->shadedBgColor;
    renderMode=guiText->renderMode;
    guiText->surfaces=surfaces_;
    guiText->surfacesY=surfacesY_;
    guiText->surfacesSize=surfacesSize_;
    guiText->shadedBgColor=shadedBgColor_;
    guiText->renderMode=renderMode_;
}
...
#ifdef _SDL_TTF_H
    TTF_Font **fonts_;
    unsigned int fontsSize_, fontsMinSize_;
    bool globalKerning_;
    int globalStyle_, globalHinting_;
    fonts_=fonts;
    fontsSize_=fontsSize;
    fontsMinSize_=fontsMinSize;
    globalKerning_=globalKerning;
    globalStyle_=globalStyle;
    globalHinting_=globalHinting;
    fonts=guiText->fonts;
    fontsSize=guiText->fontsSize;
    fontsMinSize=guiText->fontsMinSize;
    globalKerning=guiText->globalKerning;
    globalStyle=guiText->globalStyle;
    globalHinting=guiText->globalHinting;
    guiText->fonts=fonts_;
    guiText->fontsSize=fontsSize_;
    guiText->fontsMinSize=fontsMinSize_;
    guiText->globalKerning=globalKerning_;
    guiText->globalStyle=globalStyle_;
    guiText->globalHinting=globalHinting_;
#else
    bool globalKerning_;
    int globalStyle_, globalHinting_;
    globalKerning_=globalKerning;
...
TTF_Font* GUIText::GetFont(int size)
{
    if(fontsSize>0&&fonts!=NULL) {
        ...
        else if(size-fontsMinSize>=0&&size-fontsMinSize<fontsSize)return fonts[size-fontsMinSize];
        ...
        else if(size-GUITextDefault.fontsMinSize>=0&&size-GUITextDefault.fontsMinSize<GUITextDefault.fontsSize)return GUITextDefault.fonts[size-GUITextDefault.fontsMinSize];
    }
    return NULL;
}
...
void GUIDraw(SDL_Surface *dst)
{
    for(size_t n=0; n<GUIElementList_size; n++) {
        ...
        case GUI_BUTTON:
            ((GUIButton*)(GUIElementList[n].element))->Draw(dst);
            break;
        case GUI_LABEL:
            ((GUILabel*)(GUIElementList[n].element))->Draw(dst);
            break;
        case GUI_CHECKBOX:
            ((GUICheckBox*)(GUIElementList[n].element))->Draw(dst);
            break;
        case GUI_SCROLLBAR:
            ((GUIScrollBar*)(GUIElementList[n].element))->Draw(dst);
            break;
        case GUI_TEXTBOX:
            ((GUITextBox*)(GUIElementList[n].element))->Draw(dst);
            break;
        case GUI_LISTBOX:
            ...
            break;
        case GUI_DROPDOWN:
            ((GUIDropDown*)(GUIElementList[n].element))->Draw(dst);
            break;
        default:
            break;
        }
...
bool GUIEvent(SDL_Event *event, int offX, int offY)
{
    for(size_t n=0; n<GUIElementList_size; n++) {
        switch(GUIElementList[n].type) {
        case GUI_BUTTON:
            if(((GUIButton*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        case GUI_LABEL:
            if(((GUILabel*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        case GUI_CHECKBOX:
            if(((GUICheckBox*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        case GUI_SCROLLBAR:
            if(((GUIScrollBar*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        case GUI_TEXTBOX:
            if(((GUITextBox*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        case GUI_LISTBOX:
            if(((GUIListBox*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        case GUI_DROPDOWN:
            if(((GUIDropDown*)(GUIElementList[n].element))->Event(event,offX,offY))return true;
            break;
        default:
            break;
        }
