...
struct Map
{
	...
	void Resize(int w, int h, int defaultID, bool defaultSolid) {
		int ow = mapID.size(), oh = 0;
		if(ow > 0)oh = mapID.at(0).size();
		mapID.resize(w);
		mapSolid.resize(w);
		for(int n = 0; n<w; n++) {
			mapID.at(n).resize(h);
...
int main(int argc, char* args[])
{
	...
	if(FileExists("res/map.txt")) {
		...
		map.spaceX = 64;
		map.spaceY = 64;
		map.backgroundColor = (int)0xFFFFFFFF;
	...
	GUIButton increaseX, decreaseX, increaseY, decreaseY;
	increaseX.Register();
	decreaseX.Register();
	increaseY.Register();
	decreaseY.Register();
	GUIButton increaseSX, decreaseSX, increaseSY, decreaseSY;
	increaseSX.Register();
	decreaseSX.Register();
	increaseSY.Register();
	decreaseSY.Register();
	GUIButton increaseSPX, decreaseSPX, increaseSPY, decreaseSPY;
	increaseSPX.Register();
	decreaseSPX.Register();
	increaseSPY.Register();
	decreaseSPY.Register();
	GUIButton saveMap;
	saveMap.Register();
	GUIButton nextTile, prevTile;
	nextTile.Register();
	prevTile.Register();
	...
	while(run) {
		while(SDL_PollEvent(&SDLEvent)) {
			...
			case SDL_VIDEORESIZE:
				...
				drawPosX1 = map.spaceX + 20;
				drawPosY1 = 110;
				drawPosX2 = screen->w-30;
				drawPosY2 = screen->h-130;

				scrollY.SetPos(drawPosX2,drawPosY1,drawPosX2+20,drawPosY2);
				scrollX.SetPos(drawPosX1,drawPosY2,drawPosX2,drawPosY2+20);

				saveMap.SetPos(370,50,480,70);
				saveMap.SetText(" SAVE MAP ");

				increaseX.SetPos(10 ,20,90 ,40);
				increaseX.SetText(" Width++ ");
				increaseX.Register();
				decreaseX.SetPos(100,20,180,40);
				decreaseX.SetText(" Width-- ");
				decreaseX.Register();
				increaseY.SetPos(190,20,270,40);
				increaseY.SetText(" Height++ ");
				increaseY.Register();
				decreaseY.SetPos(280,20,360,40);
				decreaseY.SetText(" Height-- ");
				decreaseY.Register();

				increaseSX.SetPos(10 ,50,90 ,70);
				increaseSX.SetText(" SpaceX++ ");
				increaseSX.Register();
				decreaseSX.SetPos(100,50,180,70);
				decreaseSX.SetText(" SpaceX-- ");
				decreaseSX.Register();
				increaseSY.SetPos(190,50,270,70);
				increaseSY.SetText(" SpaceY++ ");
				increaseSY.Register();
				decreaseSY.SetPos(280,50,360,70);
				decreaseSY.SetText(" SpaceY-- ");
				decreaseSY.Register();

				increaseSPX.SetPos(10 ,80,90 ,100);
				increaseSPX.SetText(" SpawnX++ ");
				increaseSPX.Register();
				decreaseSPX.SetPos(100,80,180,100);
				decreaseSPX.SetText(" SpawnX-- ");
				decreaseSPX.Register();
				increaseSPY.SetPos(190,80,270,100);
				increaseSPY.SetText(" SpawnY++ ");
				increaseSPY.Register();
				decreaseSPY.SetPos(280,80,360,100);
				decreaseSPY.SetText(" SpawnY-- ");
				decreaseSPY.Register();

				nextTile.SetPos(10,120+map.spaceY,10+map.spaceX,140+map.spaceY);
				nextTile.SetText("Next");
				prevTile.SetPos(10,150+map.spaceY,10+map.spaceX,170+map.spaceY);
				prevTile.SetText("Prev");

				res = 10;
				setWL.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setWL.SetText("Walk left");
				res += 80;
				setWR.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setWR.SetText("Walk right");
				res += 80;
				setWU.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setWU.SetText("Walk up");
				res += 80;
				setWD.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setWD.SetText("Walk down");
				res += 80;

				setIL.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setIL.SetText("Idle left");
				res += 80;
				setIR.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setIR.SetText("Idle right");
				res += 80;
				setIU.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setIU.SetText("Idle up");
				res += 80;
				setID.SetPos(res,drawPosY2+30,res+70,drawPosY2+50);
				setID.SetText("Idle down");
				res += 80;
			...
			case SDL_KEYDOWN:
				...
				if(SDLEvent.key.keysym.sym==SDLK_LEFT) {
					selectedTile--;
					if(selectedTile<0)selectedTile = tileset.tiles.size()-1;
				}
				if(SDLEvent.key.keysym.sym==SDLK_RIGHT) {
					selectedTile++;
					if(selectedTile>=(int)tileset.tiles.size())selectedTile = 0;
				}
				break;
		...
		for(int x = 0; x<map.width; x++) {
			for(int y = 0; y<map.height; y++) {
				...
				srcX = 0;
				srcY = 0;
				if(px + pw <= drawPosX1)continue;
				if(px >= drawPosX2)continue;
				if(px < drawPosX1) {
					srcX = drawPosX1 - px;
					px = drawPosX1;
				} else if(px + map.spaceX > drawPosX2) {
					pw = drawPosX2 - px;
				}
				if(py + ph <= drawPosY1)continue;
				if(py >= drawPosY2)continue;
				if(py < drawPosY1) {
					srcY = drawPosY1 - py;
					py = drawPosY1;
				} else if(py + map.spaceY > drawPosY2) {
					ph = drawPosY2 - py;
				}
		...
		int pos = 0;
		SDLBlit(tileset.tiles.at(map.wl).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.wr).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.wu).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.wd).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.il).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.ir).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.iu).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;
		SDLBlit(tileset.tiles.at(map.id).GetSurface(),screen,10 + pos,drawPosY2+60);
		pos += map.spaceX+10;

		GUIDraw(screen);
		char txt[255];
		sprintf(txt,"Width: %d Height: %d SpaceX: %d SpaceY: %d SpawnX: %d SpawnY: %d",map.width,map.height,map.spaceX,map.spaceY,map.spawnX,map.spawnY);
		stringColor(screen,10,5,txt,0xFF0000FF);
